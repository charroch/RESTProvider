...
public class ETagSQLiteHelper extends SQLiteOpenHelper {
    ...
    private static final String ETAG_TABLE_CREATE = "CREATE TABLE IF NOT EXISTS " + TABLE_NAME
            + " (_id INTEGER NOT NULL PRIMARY KEY, " + ETAG_COLUMN + " TEXT NOT NULL UNIQUE, "
            + URL_COLUMN + " TEXT NOT NULL UNIQUE ON CONFLICT REPLACE, " + LAST_MODIFIED_COLUMN
            + " TEXT NOT NULL, " + UPDATED_COLUMN + " INTEGER NOT NULL);";

    public ETagSQLiteHelper(Context context, String name, CursorFactory factory, int version) {
    ...
    public synchronized long insertETagForUri(ETag etag, String uri) {
        ...
        if (getCount() >= MAX_ROWS) {
            ret = writableDatabase.update(TABLE_NAME, values, UPDATED_COLUMN + "=(select min("
                    + UPDATED_COLUMN + ") from " + TABLE_NAME + ")", null);
        } else {
    ...
    public synchronized ETag getETag(String uri) {
        ...
        Cursor cur = readableDatabase.query(TABLE_NAME, new String[] {
                ETAG_COLUMN, LAST_MODIFIED_COLUMN
        }, URL_COLUMN + "=?", new String[] {
            uri
        }, null, null, null);
        if (cur.moveToFirst()) {
            ETag etag = new ETag();
    ...
    /* package */int getCount() {
        Cursor cur = getReadableDatabase().rawQuery("SELECT COUNT(*) FROM " + TABLE_NAME + ";",
                null);
        cur.moveToFirst();
        try {
            return cur.getInt(0);
